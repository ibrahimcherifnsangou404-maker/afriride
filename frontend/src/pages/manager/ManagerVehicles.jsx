    ...
    <div className="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 group">
      ...
      <div className="h-56 relative overflow-hidden bg-slate-100">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          <span className={`px-3 py-1 rounded-full text-xs font-bold backdrop-blur-md shadow-sm ${vehicle.isAvailable
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-2">
          <div>
            ...
            <p className="text-slate-500 font-medium">{vehicle.category?.name || 'Non classé'}</p>
          </div>
          <div className="text-right">
            <p className="text-lg font-black text-blue-600">
              {parseFloat(vehicle.pricePerDay).toLocaleString()} <span className="text-xs font-bold text-slate-400 uppercase">FCFA/j</span>
            </p>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 sm:p-6 lg:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">Gérez votre flotte et ses disponibilités</p>
          </div>
          <Link
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-slate-100 shadow-sm">
            ...
            <p className="text-slate-500 mb-8 max-w-md mx-auto">
              {searchQuery ? 'Aucun résultat pour votre recherche.' : 'Votre agence n\'a pas encore de véhicules enregistrés.'}
            </p>
            {!searchQuery && (
              <Link
                to="/manager/vehicles/add"
      ...
      <ConfirmModal
        ...
        message={`Êtes-vous sûr de vouloir supprimer "${deleteModal.vehicleName}" ?`}
        loading={deleting}
      />
    </div>
  );
