...
export default function ManagerBookings() {
  ...
  const loadBookings = useCallback(async () => {
    try {
      ...
      setToasts((prev) => [...prev, { id, message: 'Erreur lors du chargement des réservations', type: 'error' }]);
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">Gérez et suivez toutes les locations</p>
          </div>
              ...
              >
                {status === 'all' ? 'Toutes' :
                  status === 'pending' ? 'En attente' :
                    status === 'confirmed' ? 'Confirmées' : 'En cours'}
              </button>
            ))}
          </div>
        </div>
